  ...
  chapters: [
    {
      ...
      content: `ice was beginning to get very tired of sitting by her sister on the
...
export async function getChapter(bookId: string, chapterId: string): Promise<Chapter | null> {
  if (bookId === demoBook.id) {
    return demoBook.chapters.find(c => c.id === chapterId) || null;
  }
  return null;
}

export async function getBook(bookId: string): Promise<Book | null> {
  if (bookId === demoBook.id) return demoBook;
  return null;
}